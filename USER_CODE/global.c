...
uint32      ad_raw_data,flag_new_data,stable_flag;   //数据相关变量
...
uint32      cal_ddlv_buf[6][2];   //保存系统校准的数据 得到4个常数
 ...
 void cal_calculate_old(void)
  {
	  ...
	  // 此处的校准常量 是指经过运算的数据
	  //校准电阻*(该电阻对应的AD + 每个欧姆对应的AD跨度)
	  for(i=0;i<6;i++)
      cal_ddlv_CONS[i] = cal_res_data[i][0] * (cal_ddlv_buf[i][0] + cal_ddlv_k[i]); //使用低位校准电阻作为总常量
      //得到偏移量
...
{   
    ...
    #if SDJ_SOFT 
    if(1==version_811)  
     Put_Str(280,160,Language_Str("P811型 酸度计",    "P811  PH meter"));
    else
     Put_Str(280,160,Language_Str("P801型 酸度计",    "P801  PH meter"));
  	#else
  	if(1==version_811)  
...
void  ADS1100_Match_Proc(void)
   { 
     ...
     static uint32 change_cnt1 = 0;      //用于非法数据计时
     static uint32 change_cnt2 = 0;      //用于滤波计时
     static uint32 change_cnt3 = 0;      //用于切换计时
            ...
            else
             {
              ...
              if(tmp < 500)
                {   //缓慢变化阶段
                 ...
                 if(change_cnt3 > 30)  //当缓慢变化到一定程度后，间隔提出切换要求
                    {
                     need_switch_flag = 1;
                     change_cnt3      = 0;
                    }
